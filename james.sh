              ...
              if [ "$(curl -sL -H "Authorization: Basic ${credentials}" -w "%{http_code}" "${jps}/JSSResource/${endpoint}/name/${object_purged}" -o /dev/null)" == "200" ]; then
                if [ -s "james/templates/${endpoint}/${object}.xml" ]; then
                  ...
                  if [ "${endpoint}" = "osxconfigurationprofiles" ]; then
                    ...
                    xmlstarlet ed -i "/os_x_configuration_profile/general/redeploy_on_update" -t elem -n uuid -v "${existing_uuid}" "james/templates/${endpoint}/${object}.xml" | sponge "/tmp/${object}.xml"
